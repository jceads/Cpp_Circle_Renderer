...
{
    buffer_objects = {0, 0, 0};
    SetupMesh();
}

void OpenGL::Mesh::Draw(const Shader& shader)
{
    ...
    GlCall(glBindVertexArray( buffer_objects.VAO ));
    GlCall(glDrawElements( GL_TRIANGLES, indices.size(), GL_UNSIGNED_INT, 0 ));
    GlCall(glBindVertexArray( 0 ));
    GlCall(glActiveTexture( GL_TEXTURE0 ));
}

void OpenGL::Mesh::GenerateBuffer(GLuint* buff)
{
    GlCall(glGenBuffers( 1, buff ));
}

void OpenGL::Mesh::GenerateVAO(GLuint* vao)
{
...
{
    GenerateVAO(&buffer_objects.VAO);
    GenerateBuffer(&buffer_objects.VBO);
    GenerateBuffer(&buffer_objects.EBO);


    GlCall(glBindBuffer( GL_ARRAY_BUFFER, buffer_objects.VBO ));

    GlCall(
        glBufferData( GL_ARRAY_BUFFER, vertices.size() * sizeof( OpenGL::Vertex ), vertices.data(), GL_STATIC_DRAW ));
    GlCall(glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, buffer_objects.EBO ))
    GlCall(
